...
function initBasicFunctions() {
   ...
   // Stacked Cards
   // Set initial rotation degrees and stacking index
   var stackingIndex = 2;

  // Set stacking for active stacked image
   $('.single-stacked-image[data-link-status="active"]').css('z-index', stackingIndex);

   // Handle mouseenter event for navigation
   $('[data-nav-id]').on('mouseenter', function() {
      var dataNavID = $(this).attr('data-nav-id');
      var $stackedImage = $('[data-stacked-image-id="' + dataNavID + '"]');
      var stackedImageStatus = $stackedImage.attr('data-stacked-image-status');

      // If stacked image is not active, stack it
      if(stackedImageStatus == 'not-active') {
         stackingIndex++;
         $stackedImage.attr('data-stacked-image-status', 'active').siblings().attr('data-stacked-image-status', 'not-active');
         $stackedImage.css('z-index', stackingIndex);
         gsap.fromTo($stackedImage, {
            scale: 0.9,
            opacity: 0,
         },{
            scale: 1,
   ...
   // Reset stacked images on page leave
   barba.hooks.leave(() => {
      stackingIndex = 2;
   });

   // Handle mouseover event for contact col cards
   ...
   if($(window).width() > 1024) {
      if(document.querySelector(".home-header .single-vimeo-background iframe")) {
         ...
         player1.on('play', function() {
            ...
            if($('.home-header').find('#vimeo-background-index-custom-1').attr('data-vimeo-status-loaded') == 'true'){
               setTimeout(function() {
